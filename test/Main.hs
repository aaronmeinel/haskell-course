...
main = hspec $ do
  ...
  describe "suggestNextPrescription (Double)" $ do
    ...
    describe "logSet" $ do
      let baseEx = Mesocycle.MesocycleExercise
            { Mesocycle.exerciseName = "Bench Press"
            , Mesocycle.muscleGroup = WorkoutTemplate.Chest
            , Mesocycle.prescribedSets = 3
            , Mesocycle.performedSets = Nothing
          ...
          baseWorkout = Mesocycle.MesocycleWorkout
            { Mesocycle.workoutName = "Push"
            , Mesocycle.exercises = [baseEx]
            }
          baseWeek = Mesocycle.MesocycleWeek
            { Mesocycle.weekNumber = 1
            , Mesocycle.workouts = [baseWorkout]
            }
          baseMeso = Mesocycle.Mesocycle
            { Mesocycle.numWeeks = 1
            , Mesocycle.weeks = [baseWeek]
            }
      it "logs a set for an exercise, updating performedSets and performedReps" $ do
        let updated = Mesocycle.logSet baseMeso 1 "Push" "Bench Press" Nothing 3 12
            ex' = head . Mesocycle.exercises . head . Mesocycle.workouts . head $ Mesocycle.weeks updated
        Mesocycle.performedSets ex' `shouldBe` Just 3
        Mesocycle.performedReps ex' `shouldBe` Just 12
      it "overrides prescribedReps if provided" $ do
        let updated = Mesocycle.logSet baseMeso 1 "Push" "Bench Press" (Just 15) 3 15
            ex' = head . Mesocycle.exercises . head . Mesocycle.workouts . head $ Mesocycle.weeks updated
        Mesocycle.prescribedReps ex' `shouldBe` Just 15
        Mesocycle.performedReps ex' `shouldBe` Just 15
      it "does not override prescribedReps if not provided" $ do
        let updated = Mesocycle.logSet baseMeso 1 "Push" "Bench Press" Nothing 3 10
            ex' = head . Mesocycle.exercises . head . Mesocycle.workouts . head $ Mesocycle.weeks updated
        Mesocycle.prescribedReps ex' `shouldBe` Just 10

    describe "logPreExerciseFeedback and logPostExerciseFeedback" $ do
      let baseEx = Mesocycle.MesocycleExercise
            { Mesocycle.exerciseName = "Bench Press"
            , Mesocycle.muscleGroup = WorkoutTemplate.Chest
            , Mesocycle.prescribedSets = 3
      ...
      it "logs pre-exercise soreness feedback" $ do
        let updated = Mesocycle.logPreExerciseFeedback baseMeso 1 "Push" "Bench Press" Mesocycle.StillSore
            ex' = head . Mesocycle.exercises . head . Mesocycle.workouts . head $ Mesocycle.weeks updated
        Mesocycle.preFeedback ex' `shouldBe` Just (Mesocycle.PreExerciseFeedback (Just Mesocycle.StillSore))
      it "logs post-exercise feedback (joint pain, pump, workload)" $ do
